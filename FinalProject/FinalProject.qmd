...
team_performance <- read.csv("archive/laliga2023_34/accurate_pass_team.csv") %>%
  ...
  select(-Team) %>%
  as.data.frame()
# Correct maximum values for radar chart scaling
...
# Minimum values for radar chart
min_values <- data.frame(
  ...
  ExpectedGoalsConcededPerMatch = 0,
  TackleSuccess = 0
)

# Combine max, min, and data for the radar chart
radar_data <- rbind(max_values, min_values, league_averages, team_performance)

# Ensure column names match for radar chart processing
colnames(radar_data) <- colnames(max_values)
...
radarchart(
  radar_data,
  axistype = 1,
  pcol = c(rgb(1, 0, 0, 0.9), rgb(0.2, 0.8, 0.2, 0.9)),  # Red for league average, green for Atlético Madrid
  pfcol = c(rgb(1, 0, 0, 0.4), rgb(0.2, 0.8, 0.2, 0.4)),  # Semi-transparent fill
  plwd = 2,
  cglcol = "grey",
  cglty = 1,
  axislabcol = "grey",
...
legend(
  "topright",
  legend = c("League Average", "Atlético Madrid"),
  col = c(rgb(1, 0, 0, 0.9), rgb(0.2, 0.8, 0.2, 0.9)),
  pch = 15,
  bty = "n",
  pt.cex = 1.5
)
...
# Load and filter team performance data
team_performance <- read.csv("archive/laliga2023_34/accurate_pass_team.csv") %>%
  select(Team, PassSuccess = PassSuccess...) %>%
  inner_join(read.csv("archive/laliga2023_34/team_goals_per_match.csv") %>%
               select(Team, GoalsPerMatch = GoalsperMatch), by = "Team") %>%
...
# Average performance of Top 7 teams (including Atlético Madrid if applicable)
...
# Add a title
title("Performance Comparison: Atlético Madrid vs Top 7 Teams")
